...
export default function Hero() {
  ...
  return (
    <motion.section
      className="relative min-h-screen flex flex-col md:flex-row justify-center items-center py-4 px-8 md:px-20 bg-white bg-[url('/images/svgs/bg.svg')] bg-cover bg-center overflow-hidden"
      initial="hidden"
      animate="visible"
      exit="exit"
    ...
    >
      {/* Hero video */}
      <motion.div className="order-1 md:order-2 w-full md:w-full mt-10 md:mt-0 flex justify-center md:justify-end relative px-5 hidden md:block">
      ...
      <motion.div
        ...
        className="order-1 md:order-2 relative  lg:w-1/2 w-[150%] flex justify-center lg:justify-center mt-6 lg:mt-0 z-20 block md:hidden"
        onContextMenu={(e) => e.preventDefault()}
      >
        <Image
      ...
      <motion.div
        ...
        className="order-2 md:order-1 text-center md:text-left space-y-6 max-w-lg px-4 md:px-0 pl-0 md:pl-10 pt-4 md:pt-0 ml:0 md:ml-6 z-10"
      >
        <motion.h1
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          We specialize in solutions designed to boost your business forward.
        </motion.p>
        <motion.button
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
